...
def taper_dose_str(drug: str, base_dose: float, period_idx: int) -> str:
    ...
    if pct == 0:
        return "⛔ DISCONTINUED"
    adj = round(base_dose * pct / 100)
    return f"{adj} mg (↓{100 - pct}% of baseline)"
...
def compute_dose(base: float, pct, drug: str = "") -> str:
    ...
    # Threshold check: some drugs only adjusted above a minimum dose
    threshold = DRUG_DB.get(drug, {}).get("dose_adjustment_threshold")
    if threshold and base <= threshold and pct != 0:
        return f"{int(base)} mg (no change — dose ≤{threshold} mg/day)"

    if pct == 0:
        return f"{int(base)} mg" if base else "No change"
    adj   = round(base * (1 + pct / 100))
    arrow = "UP" if pct > 0 else "DOWN"
    return f"{adj} mg ({arrow} {abs(int(pct))}%)"
...
def build_me_df(selected: dict, df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for _, row in df.iterrows():
        r = {"Week": row["Week"], "Xcopri — Evening": row["Xcopri (Cenobamate)"]}
        for drug in selected:
            val = str(row[drug])
            if "mg" in val and "Switch" not in val and "—" not in val:
                try:
                    mg   = int(val.split(" mg")[0])
                    half = mg // 2
            ...
            if threshold and info.get("dose"):
                ...
                if actual_dose <= threshold:
                    html += (
                        ...
                        f"no proactive dose reduction required. Monitor clinically.</span>"
                    )
                else:
                    html += (
                        f"<br><span style='color:#B71C1C;font-weight:500;font-size:12px'>"
...
with tab4:
    ...
    if not sd:
        ...
        st.dataframe(build_me_df(sd_no_taper, base_df), use_container_width=True, hide_index=True)
