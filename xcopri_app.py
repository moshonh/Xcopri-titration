...
def taper_dose_str(drug: str, base_dose: float, period_idx: int) -> str:
    ...
    adj = nearest_tablet(raw, sizes) if sizes else round(raw)
    return f"{adj} mg (↓{100 - pct}%)"
...
def nearest_tablet(mg: float, sizes: list) -> int:
    """Return the available tablet size closest to the target mg."""
    return min(sizes, key=lambda s: abs(s - mg))

def compute_dose(base: float, pct, drug: str = "") -> str:
    """Return adjusted dose string, rounded to nearest available tablet size."""
    if pct is None:
